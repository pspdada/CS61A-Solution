...
def balanced(m) -> bool:
    ...
    if is_planet(m):
        return True
    elif length(l) * total_mass(l) != length(r) * total_mass(r):
        return False
    else:
        return True and balanced(end(l)) and balanced(end(r))
...
def berry_finder(t):
    ...
    if label(t) == "berry":
        ...
        is_exist = [berry_finder(b) for b in branches(t)]
        if True in is_exist:
            return True
        else:
            return False
...
def max_path_sum(t):
    ...
    def find_path(t, now_value) -> int:
        now_value += label(t)
        if is_leaf(t):
            return now_value
        else:
            return max(find_path(b, now_value) for b in branches(t))

    return find_path(t, 0)
...
def find_middle(start, end):
    numbers = {1, 2, 3}
    used_numbers = {start, end}
    middle = list(numbers - used_numbers)[0]
    return middle
