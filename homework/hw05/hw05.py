...
def has_path(t: Tree, term: str) -> bool:
    ...
    if len(term) == 1:
        ...
        return True in [has_path(branch, term[1:]) for branch in t.branches]


def duplicate_link(lnk: Link, val: int) -> None:
...
def deep_map_mut(fn: Callable, lnk: Link) -> None:
