...
def double_eights(n):
    ...
    if n < 10:
        ...
        return (b == 8 and c == 8) or double_eights(a)


def make_onion(f: Callable[[int], int], g: Callable[[int], int]):
...
def mario_number(level):
    ...
    # Solution 2
    def ways(now_at):
        if now_at == len(level) - 1:
            return 1
        if now_at >= len(level) or level[now_at] == "P":
            return 0
        return ways(now_at + 1) + ways(now_at + 2)
