from scheme_builtins import scheme_pairp
from scheme_classes import Frame, LambdaProcedure, MuProcedure, Pair, SchemeError, nil
from scheme_eval_apply import eval_all, scheme_eval
from scheme_utils import is_scheme_false, is_scheme_true, scheme_listp, scheme_symbolp, validate_form, validate_formals
...
def do_quote_form(expressions: Pair, env):
...
def do_lambda_form(expressions: Pair, env):
...
def do_or_form(expressions, env):
    ...
    if expressions is nil:
        return False
    res = scheme_eval(expressions.first, env)
    if is_scheme_true(res) or expressions.rest is nil:
        return res
    else:
...
def make_let_frame(bindings, env: Frame):
    ...
    while bindings is not nil:
        binding: Pair = bindings.first
        if not scheme_listp(binding) or len(binding) != 2:
            raise SchemeError("bad binding in let form")
        name = binding.first
